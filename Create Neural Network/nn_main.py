...
train_ds = keras.utils.image_dataset_from_directory(
    ...
    batch_size=20, #Definiert die Grösse eines Batches
...
# Anwendung der Funktion auf die Trainings- und Validierungsbilder
...
def apply_Model():
    model = keras.Sequential([ # Modell definieren durch "Sequential"(gruppiert mehrere Schichten)    
        layers.Conv2D(32, kernel_size=(3, 3), activation="relu", input_shape = (50, 50, 3)), # Eine Convolutional-Schicht mit 32 3x3 Filter, mit der Aktivierungsfunktion "ReLu" (f(x)=max(0,x)) und mit einer Eingabeform des Bildes von 50x50x3 (RGB).
    ...
    histroy = model.compile(optimizer= keras.optimizers.Adam(learning_rate = 0.001), # Definiert den Optimierer des Modells als "adam" (eine stochastische Gradientenabstiegsmethode, die auf der adaptiven Schätzung von Momenten erster und zweiter Ordnung basiert) mit Learning-Rate von 0.0005
